import { useState, useEffect } from "react";
import { useSearchParams, useNavigate } from "react-router-dom";
import { setupPasswordAPI } from "../services/auth-services/setUpPasswordAPI";

export default function SetupPassword() {
    const [searchParams] = useSearchParams();
    const navigate = useNavigate();
    const token = searchParams.get("token");

    const [formData, setFormData] = useState({ password: "", confirmPassword: "" });
    const [status, setStatus] = useState("idle");
    const [errorMessage, setErrorMessage] = useState("");
    const [showPw1, setShowPw1] = useState(false);
    const [showPw2, setShowPw2] = useState(false);

    useEffect(() => {
        if (!token) navigate("/");
    }, [token, navigate]);

    const getStrength = (pw) => {
        let score = 0;
        if (pw.length >= 8) score++;
        if (/[A-Z]/.test(pw)) score++;
        if (/[0-9]/.test(pw)) score++;
        if (/[^A-Za-z0-9]/.test(pw)) score++;
        return score;
    };

    const strengthMeta = [
        { label: "Weak", color: "#E24B4A" },
        { label: "Fair", color: "#EF9F27" },
        { label: "Strong", color: "#2E7D32" },
        { label: "Very strong", color: "#1B5E20" },
    ];

    const score = getStrength(formData.password);
    const passwordsMatch = formData.password && formData.confirmPassword && formData.password === formData.confirmPassword;
    const passwordsMismatch = formData.confirmPassword && formData.password !== formData.confirmPassword;

    const handleChange = (e) => {
        setFormData({ ...formData, [e.target.name]: e.target.value });
        if (status === "error") setStatus("idle");
    };

    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!formData.password || !formData.confirmPassword) {
            setErrorMessage("Please fill in both fields.");
            setStatus("error");
            return;
        }
        if (formData.password !== formData.confirmPassword) {
            setErrorMessage("Passwords do not match!");
            setStatus("error");
            return;
        }
        setStatus("loading");
        try {
            await setupPasswordAPI(token, formData.password);
            setStatus("success");
            setTimeout(() => navigate("/"), 3000);
        } catch (err) {
            setStatus("error");
            setErrorMessage(err.response?.data?.message || "Invalid or expired link.");
            setTimeout(() => navigate("/"), 3000);
        }
    };

    const EyeIcon = () => (
        <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
            <path d="M1 7.5C2.5 4.5 5 3 7.5 3s5 1.5 6.5 4.5C12.5 11 10 12.5 7.5 12.5S2.5 11 1 7.5z" stroke="currentColor" strokeWidth="1.3" />
            <circle cx="7.5" cy="7.5" r="2" stroke="currentColor" strokeWidth="1.3" />
        </svg>
    );

    const EyeOffIcon = () => (
        <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
            <path d="M1 7.5C2.5 4.5 5 3 7.5 3s5 1.5 6.5 4.5C12.5 11 10 12.5 7.5 12.5S2.5 11 1 7.5z" stroke="currentColor" strokeWidth="1.3" />
            <path d="M1 1l13 13" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" />
        </svg>
    );

    const LockIcon = () => (
        <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
            <rect x="2.5" y="6.5" width="10" height="7" rx="2" stroke="#BBBBBB" strokeWidth="1.4" />
            <path d="M5 6.5V4.5a2.5 2.5 0 015 0v2" stroke="#BBBBBB" strokeWidth="1.4" strokeLinecap="round" />
        </svg>
    );

    if (status === "success") {
        return (
            <div style={styles.page}>
                <div style={styles.card}>
                    <div style={styles.brand}>
                        <div style={styles.brandDot}>
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                                <circle cx="8" cy="8" r="4" fill="#C5703A" />
                                <circle cx="4" cy="4" r="2" fill="#fff" opacity="0.5" />
                            </svg>
                        </div>
                        <span style={styles.brandName}>Pulse<span style={{ color: "#C5703A" }}>HR</span></span>
                    </div>
                    <div style={styles.successCard}>
                        <div style={styles.successRing}>
                            <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
                                <path d="M7 14l5 5 9-9" stroke="#2E7D32" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
                        </div>
                        <h2 style={styles.successTitle}>Password set!</h2>
                        <p style={styles.successSub}>Your account is now active.<br />Redirecting to login in a moment…</p>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 20 }}>
                            {[0, 0.2, 0.4].map((delay, i) => (
                                <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: "#2E7D32", animation: `bounce 1s ${delay}s infinite` }} />
                            ))}
                        </div>
                    </div>
                    <style>{`@keyframes bounce { 0%,80%,100%{transform:translateY(0)} 40%{transform:translateY(-6px)} }`}</style>
                </div>
            </div>
        );
    }

    return (
        <div style={styles.page}>
            <div style={styles.card}>
                {/* Brand */}
                <div style={styles.brand}>
                    <div style={styles.brandDot}>
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                            <circle cx="8" cy="8" r="4" fill="#C5703A" />
                            <circle cx="4" cy="4" r="2" fill="#fff" opacity="0.5" />
                        </svg>
                    </div>
                    <span style={styles.brandName}>Pulse<span style={{ color: "#C5703A" }}>HR</span></span>
                </div>

                {/* Header */}
                <div style={{ marginBottom: 28 }}>
                    <div style={styles.stepBadge}>
                        <div style={styles.stepBadgeDot} />
                        Account activation
                    </div>
                    <h1 style={styles.h1}>Set your password</h1>
                    <p style={styles.sub}>Choose a strong password to activate your PulseHR account.</p>
                </div>

                <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 0 }}>
                    {/* Error Box */}
                    {status === "error" && (
                        <div style={styles.errorBox}>
                            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                                <circle cx="7" cy="7" r="6" stroke="#C62828" strokeWidth="1.5" />
                                <path d="M7 4v3.5" stroke="#C62828" strokeWidth="1.5" strokeLinecap="round" />
                                <circle cx="7" cy="10" r="0.75" fill="#C62828" />
                            </svg>
                            {errorMessage}
                        </div>
                    )}

                    {/* Password Field */}
                    <div style={styles.field}>
                        <label style={styles.label}>New password</label>
                        <div style={{ position: "relative" }}>
                            <span style={styles.inputIcon}><LockIcon /></span>
                            <input
                                type={showPw1 ? "text" : "password"}
                                name="password"
                                placeholder="••••••••"
                                style={styles.input}
                                value={formData.password}
                                onChange={handleChange}
                                required
                            />
                            <button type="button" style={styles.eyeBtn} onClick={() => setShowPw1(!showPw1)}>
                                {showPw1 ? <EyeOffIcon /> : <EyeIcon />}
                            </button>
                        </div>

                        {/* Strength Bar */}
                        <div style={{ display: "flex", gap: 4, marginTop: 8 }}>
                            {[1, 2, 3, 4].map((i) => (
                                <div key={i} style={{
                                    flex: 1, height: 3, borderRadius: 2,
                                    background: i <= score && formData.password ? strengthMeta[score - 1]?.color : "#EEE",
                                    transition: "background 0.3s"
                                }} />
                            ))}
                        </div>
                        {formData.password && (
                            <p style={{ fontSize: 11, marginTop: 5, fontWeight: 500, color: strengthMeta[score - 1]?.color }}>
                                {strengthMeta[score - 1]?.label}
                            </p>
                        )}
                    </div>

                    {/* Confirm Password Field */}
                    <div style={styles.field}>
                        <label style={styles.label}>Confirm password</label>
                        <div style={{ position: "relative" }}>
                            <span style={styles.inputIcon}><LockIcon /></span>
                            <input
                                type={showPw2 ? "text" : "password"}
                                name="confirmPassword"
                                placeholder="••••••••"
                                style={styles.input}
                                value={formData.confirmPassword}
                                onChange={handleChange}
                                required
                            />
                            <button type="button" style={styles.eyeBtn} onClick={() => setShowPw2(!showPw2)}>
                                {showPw2 ? <EyeOffIcon /> : <EyeIcon />}
                            </button>
                        </div>
                        {passwordsMatch && (
                            <p style={{ fontSize: 11, marginTop: 5, color: "#2E7D32", fontWeight: 500, display: "flex", alignItems: "center", gap: 4 }}>
                                <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2 6l3 3 5-5" stroke="#2E7D32" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                                Passwords match
                            </p>
                        )}
                        {passwordsMismatch && (
                            <p style={{ fontSize: 11, marginTop: 5, color: "#C62828", fontWeight: 500, display: "flex", alignItems: "center", gap: 4 }}>
                                <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2 2l8 8M10 2l-8 8" stroke="#C62828" strokeWidth="1.5" strokeLinecap="round" /></svg>
                                Passwords do not match
                            </p>
                        )}
                    </div>

                    <button type="submit" disabled={status === "loading"} style={styles.btn}>
                        {status === "loading" ? "Activating…" : "Activate account"}
                        {status !== "loading" && (
                            <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
                                <path d="M3 7.5h9M9 4l3.5 3.5L9 11" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
                        )}
                    </button>
                </form>

                <div style={{ height: 1, background: "#F0EDE8", margin: "20px 0" }} />
                <p style={styles.footerNote}>
                    This link is single-use and expires in 24 hours.<br />
                    Contact your admin if you need a new one.
                </p>
            </div>

            <style>{`
                @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600&family=Sora:wght@700&display=swap');
                @keyframes bounce { 0%,80%,100%{transform:translateY(0)} 40%{transform:translateY(-6px)} }
            `}</style>
        </div>
    );
}

const styles = {
    page: {
        minHeight: "100vh",
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        background: "#F0EDE8",
        padding: 20,
        fontFamily: "'DM Sans', sans-serif",
        position: "relative",
        overflow: "hidden",
    },
    card: {
        width: "100%",
        maxWidth: 420,
        background: "#fff",
        borderRadius: 24,
        padding: "40px 36px",
        border: "1px solid rgba(0,0,0,0.06)",
        position: "relative",
        zIndex: 1,
    },
    brand: {
        display: "flex",
        alignItems: "center",
        gap: 8,
        marginBottom: 32,
    },
    brandDot: {
        width: 28, height: 28,
        borderRadius: 8,
        background: "#1A1A2E",
        display: "flex", alignItems: "center", justifyContent: "center",
    },
    brandName: {
        fontFamily: "'Sora', sans-serif",
        fontSize: 15, fontWeight: 700,
        color: "#1A1A2E", letterSpacing: "-0.3px",
    },
    stepBadge: {
        display: "inline-flex", alignItems: "center", gap: 6,
        background: "#FFF3EA", border: "1px solid #F0C9A8",
        borderRadius: 20, padding: "4px 12px",
        fontSize: 11, fontWeight: 600, color: "#C5703A",
        letterSpacing: "0.5px", textTransform: "uppercase",
        marginBottom: 14,
    },
    stepBadgeDot: {
        width: 6, height: 6, borderRadius: "50%", background: "#C5703A",
    },
    h1: {
        fontFamily: "'Sora', sans-serif",
        fontSize: 22, fontWeight: 700,
        color: "#1A1A2E", letterSpacing: "-0.5px",
        marginBottom: 6, margin: "0 0 6px",
    },
    sub: {
        fontSize: 13, color: "#888", lineHeight: 1.6, margin: 0,
    },
    field: {
        display: "flex", flexDirection: "column", marginBottom: 18,
    },
    label: {
        fontSize: 12, fontWeight: 600, color: "#444",
        letterSpacing: "0.3px", marginBottom: 7,
        textTransform: "uppercase",
    },
    inputIcon: {
        position: "absolute", left: 14, top: "50%",
        transform: "translateY(-50%)",
        display: "flex", alignItems: "center", color: "#BBB",
    },
    input: {
        width: "100%", padding: "12px 48px 12px 42px",
        fontSize: 14, fontFamily: "'DM Sans', sans-serif",
        borderRadius: 12, border: "1.5px solid #E8E4E0",
        background: "#FAFAF9", color: "#1A1A2E",
        outline: "none", boxSizing: "border-box",
        letterSpacing: 2,
    },
    eyeBtn: {
        position: "absolute", right: 14, top: "50%",
        transform: "translateY(-50%)",
        background: "none", border: "none",
        cursor: "pointer", color: "#BBB",
        display: "flex", alignItems: "center", padding: 0,
    },
    btn: {
        width: "100%", padding: 14,
        background: "#1A1A2E", color: "#fff",
        border: "none", borderRadius: 12,
        fontSize: 14, fontWeight: 600,
        fontFamily: "'DM Sans', sans-serif",
        cursor: "pointer", marginTop: 8,
        display: "flex", alignItems: "center",
        justifyContent: "center", gap: 8,
        letterSpacing: "0.2px",
    },
    errorBox: {
        background: "#FFF1F0", border: "1px solid #FFCDD2",
        borderRadius: 10, padding: "10px 14px",
        fontSize: 12, color: "#C62828",
        display: "flex", alignItems: "center", gap: 8,
        marginBottom: 18,
    },
    footerNote: {
        fontSize: 11, color: "#BBB",
        textAlign: "center", lineHeight: 1.6, margin: 0,
    },
    successCard: { textAlign: "center", padding: "16px 0 8px" },
    successRing: {
        width: 72, height: 72, borderRadius: "50%",
        background: "#F0FAF0", border: "2px solid #A5D6A7",
        display: "flex", alignItems: "center", justifyContent: "center",
        margin: "0 auto 20px",
    },
    successTitle: {
        fontFamily: "'Sora', sans-serif",
        fontSize: 20, fontWeight: 700,
        color: "#1A1A2E", marginBottom: 8, margin: "0 0 8px",
    },
    successSub: { fontSize: 13, color: "#888", lineHeight: 1.6, margin: 0 },
};